...
namespace PixelToaster
{
	...
	**/
    ...
    public:
		...
		FloatingPointPixel( float r, float g, float b, float a = 0.0f ):r(r),g(g),b(b),a(a){}

//		// assist sse intrinsics
//		FloatingPointPixel( __m128 m ):v(m){}
...
assert( ! (a == b) );
    ...
    public:
		...
		virtual Framebuffer *update_begin( void ) = 0;
		virtual bool update_end( void ) = 0;
...
while ( display.open() )
{
    ...
    public:
		...
		/// XXX Experimental -- Begin a display update.
		/// Returns a Framebuffer object with the (hardware) pixel buffer.
		/// Use it to avoid memcpy, or do your own conversions (or call PT's conversions yourself).
		///
		/// You should validate the Format and stride returned
		///
		/// @returns a Framebuffer object if the buffer is ready to use, or false.
		
		Framebuffer *update_begin( void )
		{
			if (internal)
				return internal->update_begin();
			else
				return false;
		}

		/// XXX Experimental -- End a display update.
		/// "Commits" a frame that was started with update_begin()
		///
