...
extern int hackmode_enable, hackmode_height, hackmode_width;
...
namespace PixelToaster
{
	...
	public:
		...
		/// XXX begin an update -- first half of update()
		/// @returns pointer to Framebuffer or false

		Framebuffer *update_begin( void )
		{
			...
			D3DLOCKED_RECT lock;
			if ( FAILED( primaryTexture->LockRect( 0, &lock, 0, D3DLOCK_NOSYSLOCK ) ) )
				return false;

			fb.pixels = (void*)lock.pBits;
			fb.stride = lock.Pitch;
			fb.format = textureFormat;

			return &fb;
		}

		/// XXX complete and update - the second "half" of update()
		...
		bool createDevice( LPDIRECT3D9 direct3d, int width, int height, Format format, bool windowed, D3DDEVTYPE devType = D3DDEVTYPE_HAL )
		{
			...
			if ( !windowed )
			{
				...
				for ( UINT i = 0; i < n; ++i )
				{
					...
					{
//						printf("mode: % 5d x % 5d: ", mode.Width, mode.Height);
						if ( mode.Width >= (UINT)width && mode.Height >= (UINT)height && ( bestWidth == 0 || mode.Width <= (UINT)bestWidth && mode.Height <= (UINT)bestHeight ) )
						{
//							printf("candidate! ");
if ( hackmode_enable ) {
	if ( mode.Height == hackmode_height && mode.Width == hackmode_width ) {
...
//						printf("\n");
				...
				width = bestWidth;
				height = bestHeight;
//				printf("width(%d), height(%d), drawAsQuad(%s), scalesUp(%s)\n", width, height, drawAsQuad ? "true" : "false", scalesUp ? "true" : "false");
//				_getch();
			}
		...
		bool createTexture( const SmartI<IDirect3DDevice9>& device, int width, int height, Format format )
		{
			...
			if ( textureFormat == Format::XBGRFFFF || textureFormat != deviceFormat || this->drawAsQuad )
			{
...
/*
 *XXX here is a place to...
 *   allocate an internal screen-buffer (if conversion /is/ needed)
 * update_start() can return ....
 *   the internal buffer, if conversion is needed
 * or
 *   the locked dx buffer, if conversion is not needed
 */
		...
		void drawQuad()
		{
			...
			//printf("surface description w(%d),h(%d)\n", description.Width, description.Height );
...
/*
			quad[0].pos = D3DXVECTOR4(0, 0, 0.5f, 1.0f);
			quad[0].tex = D3DXVECTOR2(0.f, 0.f);

			quad[1].pos = D3DXVECTOR4(w, 0, 0.5f, 1.0f);
			quad[1].tex = D3DXVECTOR2(1.f, 0.f);

			quad[2].pos = D3DXVECTOR4(0, h, 0.5f, 1.0f);
			quad[2].tex = D3DXVECTOR2(0.f, 1.f);
